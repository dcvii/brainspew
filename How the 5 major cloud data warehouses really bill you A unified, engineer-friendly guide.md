...
source: "https://clickhouse.com/blog/how-cloud-data-warehouses-bill-you"
author:
  - "[[ClickHouse]]"
published: 2025-11-27
created: 2025-12-04
description: "This guide explains how the five major cloud data warehouses—Snowflake, Databricks, ClickHouse Cloud, BigQuery, and Redshift—allocate, meter, and bill compute, giving engineers a clear understanding of what the billing units mean and how to compare them."
tags:
  - "clippings"
---
...
![tom schreiber](https://clickhouse.com/_next/image?url=%2Fuploads%2Ftom_schreiber_aa58c99e87.jpg&w=48&q=75)

tom schreiber

![](https://clickhouse.com/_next/image?url=%2Fuploads%2Flio_headshot_singapore_7cc9852011.jpg&w=96&q=75)

Tom Schreiber & Lionel Palacin

Dec 1, 2025 -20 minutes read
...
> This post focuses entirely on **compute costs**, because that’s where the real differences between systems lie. Storage costs, by comparison, are **much simpler** and, crucially, often **negligible relative to compute**, since most systems store data in low-cost cloud object storage with very similar pricing. That said, our **Bench2Cost** framework (see next section) does fully calculate storage costs for every system. We simply don’t highlight them here because they don’t materially affect the comparison.
...
![Blog-Costs.001.png](https://clickhouse.com/uploads/Blog_Costs_001_aa09baa628.png) *(Note: Snowflake does not publish hardware specs, but it is widely [understood](https://select.dev/posts/snowflake-warehouse-sizing) that each node roughly corresponds to **8 vCPUs, 16 GiB RAM, and ~200 GB local disk cache**.)*
...
To translate Snowflake’s ClickBench runtimes into dollars, we applied our “Bench2Cost” pipeline, which joins existing **raw benchmark timings**, **warehouse configuration**, and **Snowflake’s pricing tables** into one unified cost dataset.

  
Snowflake’s ClickBench results (e.g., [4xl.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/snowflake/clickbench/results_100b/4xl.json)) contain runtimes measured on a fixed warehouse size, in this case a **4X-Large (128 credits/hour)** warehouse.

  
Separately, we encoded Snowflake’s official pricing data (warehouse T-shirt sizes, credits/hour, and per-credit dollar prices across editions and regions) into [standard\_warehouse.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/snowflake/pricings/standard_warehouse.json).

  
A script ([enrich.sh](https://clickhouse.com/blog/enrich.sh)) merges these inputs:
...
- outputs a fully enriched file ([4xl\_enriched.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/snowflake/results_100B/4xl_enriched.json)) containing:
- per-query runtime
...
![Blog-Costs.002.png](https://clickhouse.com/uploads/Blog_Costs_002_c7cc179f23.png) *(Note that Databricks abstracts the exact hardware and node counts, so the diagram is illustrative, not literal. For clarity, we omit driver nodes.)*
...
To convert Databricks’ ClickBench runtimes into dollars, we ran the same **Bench2Cost** pipeline used for Snowflake, this time joining the raw benchmark timings with Databricks’ **warehouse-size → DBUs/hour** mapping and **DBU → dollar** pricing.

  
For example, a Databricks ClickBench file ([clickbench\_4X-Large.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/databricks/clickbench/large/results_100B/clickbench_4X-Large.json)) contains runtimes measured on a fixed SQL warehouse size, here a **4X-Large** warehouse (528 DBUs/hour).

  
Separately, we encoded Databricks’ official pricing tables (cluster sizes, DBUs/hour, and DBU prices per cloud/region/tier) into [sql\_serverless\_compute.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/databricks/pricings/sql_serverless_compute.json).

  
The enrichment script ([enrich.sh](https://clickhouse.com/blog/enrich.sh)) merges these inputs and:
...
- writes out an enriched file ([clickbench\_4X-Large\_enriched.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/databricks/results_100B/clickbench_4X-Large_enriched.json)) containing
- per-query runtime
	- per-query compute cost
	- total storage cost
...
As with Snowflake and Databricks, ClickHouse Cloud goes through the same Bench2Cost pipeline. For compute we start from an existing ClickBench result file for a fixed service size, for example a **3-node, 64 GiB** service captured in [aws.3.64.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/clickhouse-cloud/results/aws.3.64.json). Separately, [we encode ClickHouse Cloud list prices](https://github.com/ClickHouse/examples/tree/main/blog-examples/Bench2Cost/clickhouse-cloud/pricings) per **compute unit / hour** and per **TB-month of storage** for each tier (Basic, Scale, Enterprise).

  
The Bench2Cost script ([enrich.sh](https://clickhouse.com/blog/enrich.sh)) then:
...
- writes out an enriched JSON file ([aws.3.64\_enriched.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/clickhouse-cloud/results/aws.3.64.json)) that includes, for each tier:
- per-query runtimes,
...
We added a BigQuery-specific runner ([run\_bq\_bench.sh](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/bigquery/clickbench/bigquery_extended/run_bq_bench.sh)) that wraps each query execution in the BigQuery CLI and extracts three things:
...
We created a pricing descriptor file, [serverless.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/bigquery/pricings/serverless.json), containing:

  
- **Capacity-based compute pricing  
	**(Standard / Enterprise / Enterprise Plus), with per-slot-second rates per region
...
A dedicated script ([enrich.sh](https://clickhouse.com/blog/enrich.sh)) takes:

- The raw benchmark results ([result.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/bigquery/clickbench/bigquery_extended/results/result.json))
- The pricing metadata ([serverless.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/bigquery/pricings/serverless.json))

It then computes (and outputs into [result\_enriched.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/bigquery/results/result_enriched.json)):
...
![Blog-Costs.006.png](https://clickhouse.com/uploads/Blog_Costs_006_4f05cc9507.png) *([Each RPU is equivalent to 16 GB memory and 2 vCPU](https://medium.com/@surojitchowdhury/exploring-aws-redshift-serverless-a-closer-look-59e531790722))*
...
That extension lives in [get\_metrics.sh](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/redshift-serverless/clickbench/redshift-serverless_extended/get_metrics.sh)

  
With that in place, the pipeline works like this:

  
1. **Re-run ClickBench with the extended runner  
	**This produced a Redshift-specific result file containing both the usual wall-clock runtimes **and** the RPU-time for every execution: e.g. [serverless\_100m.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/redshift-serverless/clickbench/redshift-serverless_extended/results/serverless_100m.json)
2. **Load Redshift’s pricing model  
	**Bench2Cost uses a pricing descriptor ([serverless.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/redshift-serverless/pricings/serverless.json)) that contains RPU rates and storage pricing.
3. **Enrich results with dollar costs  
	**The enrich.sh script joins the benchmark results with the pricing model and emits a fully enriched JSON file containing per-query compute cost (based on RPU-time) and storage cost: e.g. [enriched\_100m.json](https://github.com/ClickHouse/examples/blob/main/blog-examples/Bench2Cost/redshift-serverless/results/enriched_100m.json)
