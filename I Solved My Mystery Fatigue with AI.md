...
created: 2026-06-17
description: "Here’s how you can do the same"
tags:
  - "brain_spew"
---
...
Welcome to the diagnostic game I’ve been playing. Last year, I was [diagnosed with](https://metalearn.substack.com/p/what-i-learnt-from-having-a-brain) a prolactinoma—a tumor on the pituitary gland in the center of my head. [Two brain surgeries](https://metalearn.substack.com/p/a-sequel-nobody-asked-for) last August and November couldn’t fully remove it (you can find my partner Andrew’s public research on my care [here](https://substack.com/@andrewjrod/p-186112551)). When a drug successfully controlled the tumor’s growth, I couldn’t be more excited. And then these episodes started. I had no idea how to make them stop until I cracked the mystery by running a process with AI. Now, I’ve been feeling consistently good for a month:)

![](https://substackcdn.com/image/fetch/$s_!ptbn!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F9cd6e0ef-2f41-4156-b590-b2e043116c1d_4130x4256.jpeg)
...
To be clear, no model outperformed my neuroendocrinologist, the country’s top expert in my condition. The models didn’t anticipate his hypothesis, and I gained more clarity in 20 minutes with him than in the dozens of hours I spent eliciting them. But they did raise nearly every hypothesis [^1] his NP offered on a phone visit, and even flagged a specialized test the NP independently ordered for me. Unsurprisingly, they also easily beat every PCP I’ve seen.
...
![](https://substackcdn.com/image/fetch/$s_!pWve!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fd034c828-9712-445b-97d9-322505532898_1672x941.png)
...
This process turns your mystery symptoms into trackable, verifiable tasks—the kind of work AI does best. If you’d rather skip ahead, **the [appendix](https://metalearn.substack.com/i/201330336/appendix-i-plug-and-play-artifacts) has a plug-and-play system prompt and a coding-agent skill**, both built around this method. But the step-by-step sections below cover details the artifacts leave out.
...
![](https://substackcdn.com/image/fetch/$s_!8oud!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Feb2f5fe3-1daa-46a5-918a-a4aad446d292_2458x1060.png)

- For advanced users: try a coding agent (like Codex or Claude Code). The advantages are:
	1. They handle complex file formats (e.g., CT / MRI images) and large data dumps (e.g., months of Garmin data) better.
		2. Their planning mode can surface unknown-unknowns, which is critical in diagnosis.
		3. Instead of re-uploading a file every time it changes, you can keep all your data in a local folder and edit it in place.
		4. You can build custom tools and reminders for your symptom tracker

#### Provide ALL the context

No amount of context is too much. This is true whether you’re seeing a PCP and talking to a model. However, [an average doctor only lets their patient talk for ~11s before interrupting](https://link.springer.com/article/10.1007/s11606-018-4540-5). Models have infinite time and patience. So, be liberal and give it everything!

1. Write up your medical history, including gender, age, weight, height, family history, and other information that you think matters. Share this as a text snippet to the model.
2. Attach all potentially relevant records: blood work results, specialty tests, past clinical notes, etc. ChatGPT Health allows you to connect certain EHR systems directly; otherwise, these records can usually be exported from your electronic healthcare system. You can simply ask an LLM how to do so!
3. Keep adding context as you go. New data surfaces throughout the four-step process, so pause periodically and ask what else might be relevant. It’s more likely you’ve missed something than not. The model and your doctor should ask the questions that surface your unknown-unknowns, but with such a narrow view into your life, they’ll often miss things you’d catch.
4. Have data privacy concerns? Both ChatGPT and Claude let you opt out of having your data used for training; however, they are [not by default HIPAA-compliant](https://www.hipaajournal.com/is-chatgpt-hipaa-compliant/) right now. I am happy to share my data because the upside of improving my day-to-day is high enough to outweigh the concern. This is a view that other [patients](https://osteosarc.com/) and [advocates](https://www.statnews.com/2026/01/29/chatgpt-health-review-privacy-trust/) share, but it’s ultimately a personal decision.
...
![](https://substackcdn.com/image/fetch/$s_!WVt8!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fd772713c-4b1d-412f-a83f-6c7f8215ac38_1672x941.png)
...
- **General blood work.** I used [Function Health](https://www.functionhealth.com/) for a panel covering 100+ biomarkers at $365 a year. It’s pricier than asking a PCP for individual tests, but it was the most efficient way—in cost and time—to get broad coverage at once.
- **Specialized testing.** Given my pituitary history, I asked my endocrinologist for a full pituitary hormone panel. Research with the models led me to suspect an HPA-axis issue, which calls for an ACTH stimulation test—the same one my NP later suggested independently [^3].
...
![](https://substackcdn.com/image/fetch/$s_!SoF1!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F7fe67c71-59a0-411d-9106-0ce5326d2e15_676x576.png)
...
- **All my test results:** Function Health [^4], the specialized pituitary labs [^5], gut health results, and at-home blood pressure readings.
...
![](https://substackcdn.com/image/fetch/$s_!zTGy!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F1353de80-6275-46b7-bff8-b9d989e7f180_2027x776.png)
...
Your supporting cast might look different: a physical therapist, a sleep coach, a psychologist. Some services even offer essentially free [^6] initial visits. Never underestimate what one of these conversations can surface. Worst case, you rule out one more possible cause, which is still useful progress.
...
1. **Definitely worth doing.** Taking iron when ferritin is below range is recommended [^7] whether or not it’s the cause of my fatigue. Staying hydrated is likewise a no-brainer. Start these right away since there’s usually no need to worry about them overlapping with other experiments.
2. **Harmless but maybe useless.** Creatine is in this category. It may do little, but it could confound experiments running at the same time and make my real symptoms harder to read. It’s often worth spacing experiments like this out so you can isolate their effects.
3. **Potentially risky or costly.** Changing a medication dose or stopping exercise both belong here. For these, consult your physician and keep the change time-bounded: when I stopped my medication and my exercise, I had a plan to resume and a clear sense of what I was testing. Run these one at a time.
...
## If you’re a scientist, physician, patient, researcher, or building in Health AI, reach out!

**Here’s my [calendar](https://calendar.app.google/Psc3DGJ8iiZqL5pu5); I’d love to help you make better use of AI.** Grab time before ideas and thoughts escape you:)
...
- **If you’re a patient**, I’m happy to offer thoughts on how to elicit the models for your research—but only after you’ve tried the basics in the prompting section and the prompts or skills I linked in the [appendix](https://metalearn.substack.com/i/201330336/appendix-i-plug-and-play-artifacts):)
...
[System prompt](https://github.com/amydeng2000/health-self-investigation-skill/blob/main/skills/health-self-investigation/references/portable-core.md), [Skill for Codex and Claude Code](https://github.com/amydeng2000/health-self-investigation-skill)
...
![](https://substackcdn.com/image/fetch/$s_!yYMk!,w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Fffe6f495-87b4-428e-9bc2-259c16505729_2516x1258.png)

And if you’re comfortable with coding agents like Codex or Claude Code, here’s a [skill](https://github.com/amydeng2000/health-self-investigation-skill) I built that starts with a short symptom intake and routes you to the right phase of the four-step process. It takes a little more setup but is more comprehensive than the system prompt alone.

## Appendix II: Bulid Your Personal Health Stack
...
Have too much data across hospitals, labs, and devices, and need to catch a new doctor up on your past treatments? Build a website to visualize things, as GitLab co-founder Sid did with his own cancer data [here](https://osteosarc.com/).
...
Thank you [Jane](https://thoughtsbyjane.substack.com/), Rahul, Eli, Luke, and [Andrew](https://substack.com/@andrewjrod) for thoughts and feedback on this piece <3

[^1]: The 1 out of 5-6 hypotheses the model didn’t bring up was one that was the least relevant.

[^2]: The models aren’t good at estimating quantity from a photo alone—in my experience, the error can be up to 2x off in both directions.

[^3]: It’s very cool that the models landed on the same niche test a trained specialist did.

[^4]: Function Health doesn’t let you bulk-export results until a clinician writes a summary, which takes weeks. So I had Codex use browser automation to read the results straight off my screen once I’d logged in, and pull every abnormal test’s name, value, and reference range, plus brief summaries of the in-range results.

[^5]: MyChart doesn’t let you bulk export test results either, so I asked an LLM how to circumvent this. I ended up connecting MyChart to Apple Health, which automatically compiles your lab results in PDFs

[^6]: Fay, where I found my dietitian, gives you an upfront cost estimate; if a dietitian is in-network, that estimate is often $0, and even if you’re later charged a co-pay, Fay refunds you to match the original estimate.

[^7]: Barring a conflicting condition, which I don’t have

[^8]: For a long time, my estrogen sat at postmenopausal levels, which put me in the bottom 5% of bone density for my age and likely pushed visceral fat toward my abdomen.