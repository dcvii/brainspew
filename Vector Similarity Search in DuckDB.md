...
tags:
  ...
  - duckdb
---
In DuckDB v0.10.0, we introduced the [`ARRAY` data type](https://duckdb.org/docs/stable/sql/data_types/array.html), which stores fixed-sized lists, to complement the existing variable-size [`LIST` data type](https://duckdb.org/docs/stable/sql/data_types/list.html).
...
However, as the hype for **vector embeddings** and **semantic similarity search** was growing, we also snuck in a couple of distance metric functions for this new `ARRAY` type:[`array_distance`](https://duckdb.org/docs/stable/sql/functions/array.html#array_distancearray1-array2),[`array_negative_inner_product`](https://duckdb.org/docs/stable/sql/functions/array.html#array_negative_inner_productarray1-array2) and [`array_cosine_distance`](https://duckdb.org/docs/stable/sql/functions/array.html#array_cosine_distancearray1-array2)
...
We're actively working on addressing this and other issues related to index persistence, which will hopefully make it into [DuckDB v0.10.3](https://duckdb.org/release_calendar.html#upcoming-releases), but for now we recommend using the `HNSW` index in in-memory databases only.
...
While we're still working on addressing some of the limitations above, particularly those related to persistence (and performance), we still really want to share this early version the `vss` extension as we believe this will open up a lot of cool opportunities for the community. So make sure to check out the [`vss` extension documentation](https://duckdb.org/docs/stable/core_extensions/vss.html) for more information on how to work with this extension!
