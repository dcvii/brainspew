...
source: "https://x.com/zostaff/status/2054533153893613839"
author:
  - "[[@zostaff]]"
published: 2026-05-13
created: 2026-05-16
description: "Backtest engine, feature pipeline, validation stack, AI research loop, production monitoring, all code, no filler.In 2018 a serious quantita..."
tags:
  - "brain_spew"
---
![Image](https://pbs.twimg.com/media/HIJxYPEXUAAyuFf?format=jpg&name=large)
...
But faster generation without faster validation is not a 50x productivity gain. It is a 50x amplifier of statistical garbage. This article is the engineer's manual for that new world, almost entirely code, with just enough conceptual scaffolding to understand why each piece exists. **Everything described here is packaged into a working repository:** [https://github.com/zostaff/ai-quant-researcher](https://github.com/zostaff/ai-quant-researcher)

# Architecture

![Image](https://pbs.twimg.com/media/HIFH2oeXMAEA8gi?format=jpg&name=large)
...
**Constant \`fee\_bps\` is a lie. Real costs depend on size, volatility, and venue:**
...
Sweep \`order\_size\` with this model. Your "Sharpe 2" at $100k becomes Sharpe 0.4 at . That's your strategy's capacity.

# Features and leakage

![Image](https://pbs.twimg.com/media/HIFJvPYWcAAvXfL?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HIFKhRQWMAAha29?format=jpg&name=large)
...
![Image](https://pbs.twimg.com/media/HIFLKcXWEAAMCTF?format=jpg&name=large)
...
Hard gate: \`dsr\_pvalue < 0.95\` = strategy does not ship. No human override.

## AI research loop with Claude

![Image](https://pbs.twimg.com/media/HIFMgcyWoAA0f-0?format=jpg&name=large)
...
The entire pipeline from this article is packaged as an open-source repository: [https://github.com/zostaff/ai-quant-researcher](https://github.com/zostaff/ai-quant-researcher) This is not a trading framework - it is a research validation engine: Claude generates strategies, the system validates them and kills the bad ones. Inside: vectorized and event-driven backtest (with slippage and partial fills), a feature pipeline with a leakage detector (correlation + structural via truncation), walk-forward with purge, combinatorial purged CV, and Deflated Sharpe Ratio as a hard gate with no override - something none of TradingAgents, AgentQuant, or QuantEvolve have. Five Claude agents (Hypothesis, Code, Critic with per-market templates, Risk, Memory on SQLite), a three-gate evaluator, an AST-validated sandbox for LLM code execution, and a kill-switch for production. 54 tests, GitHub Actions CI, ~4500 lines, 5 dependencies. Examples 1-5 run without an API key on synthetic data - you can immediately see how DSR kills the best-of-1,000 random strategies. To turn the skeleton into a live tool, you need real data (Polygon/Databento) and a broker API - the code is ready for both.

**The tools are extraordinary. The temptation to deploy without validation is extraordinary. The cost of getting it wrong, when AI lets you get it wrong faster than ever, is extraordinary. Build slow. Validate hard. Trade small until you've proven yourself, to yourself.**