...
source: "https://duckdb.org/2025/08/08/spatial-joins.html"
author:
  - "[[Max Gabrielsson]]"
published: 2025-08-07
created: 2025-08-11
description: "DuckDB v1.3.0 significantly improved the scalability of geospatial joins with a dedicated SPATIAL_JOIN operator."
tags:
  - "clippings"
---
...
Ever since its inception, DuckDB's `spatial` extension has provided a `GEOMETRY` column type to represent locations, regions and shapes, along with plenty of *spatial predicate* functions to use when `JOIN` ing. However, it wasn't until recently in [DuckDB v1.3.0](https://duckdb.org/2025/05/21/announcing-duckdb-130.html) that spatial joins really became *scalable* thanks to the introduction of the dedicated `SPATIAL_JOIN` query **operator**.

We managed to hastily sneak in some information about this new operator towards [the end of the v1.3.0 release notes](https://duckdb.org/2025/05/21/announcing-duckdb-130.html#spatial-join-operator), but we figured we got enough to talk about to justify a separate blog post. So in this post, we will take a closer look at some of the challenges involved in optimizing spatial joins, and how the new `SPATIAL_JOIN` operator addresses them to achieve new levels of efficiency.
...
| --- | --- |
| ```sql ST_Intersects(a, b) ``` | Whether `a` intersects `b` |
| ```sql ST_Contains(a, b) ``` | Whether `a` contains `b` |
| ```sql ST_ContainsProperly(a, b) ``` | Whether `a` contains `b` without `b` touching `a` 's boundary |
| ```sql ST_Within(a, b) ``` | Whether `a` is within `b` |
| ```sql ST_Overlaps(a, b) ``` | Whether `a` overlaps `b` |
| ```sql ST_Touches(a, b) ``` | Whether `a` touches `b` |
| ```sql ST_Equals(a, b) ``` | Whether `a` is equal to `b` |
| ```sql ST_Crosses(a, b) ``` | Whether `a` crosses `b` |
| ```sql ST_Covers(a, b) ``` | Whether `a` covers `b` |
| ```sql ST_CoveredBy(a, b) ``` | Whether `a` is covered by `b` |
| ```sql ST_DWithin(a, b, x) ``` | Whether `a` is within distance `x` of `b` |
...
To show how this affects the query plan, we're going to go [back in time](https://duckdb.org/release_calendar.html#past-releases) and use DuckDB **v1.2.0** (which does contain the `PIECEWISE_MERGE_JOIN` optimization, but **not** the `SPATIAL_JOIN` operator) to re-run the query from above.
...
```sql
JOIN ... ON ST_Intersects(a.geom, b.geom) AND a.id = b.id
```
.
