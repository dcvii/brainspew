...
source: "https://duckdb.org/2025/05/21/announcing-duckdb-130.html?utm_source=tldrdata"
author:
  - "[[The DuckDB team]]"
published: 2025-05-20
created: 2025-05-26
description: "The DuckDB team is happy to announce that today we're releasing DuckDB version 1.3.0, codenamed “Ossivalis”."
tags:
  - "clippings"
---
*TL;DR: The DuckDB team is happy to announce that today we're releasing DuckDB version 1.3.0, codenamed “Ossivalis”.*

> To install the new version, please visit the [installation guide](https://duckdb.org/docs/installation/). Note that it can take a few hours to days to release some client libraries (e.g., Go, R, Java) and extensions (e.g., the UI) due to the extra changes and review rounds required.
...
In this blog post, we cover the most important features of the new release. DuckDB is moving rather quickly, and we could cover only a small fraction of the changes in this release. For the complete release notes, see the .
...
We highly value [portability](https://duckdb.org/why_duckdb.html#portable), so it is of course still possible to [build DuckDB from source](https://duckdb.org/docs/stable/dev/building/overview.html) for older versions of glibc.

### Lambda Function Syntax

Previously, lambda functions in DuckDB could be specified using the single arrow syntax: `x -> x + 1`. The single arrow operator is also used by the JSON extension to express [JSON extraction](https://duckdb.org/docs/stable/data/json/json_functions.html#json-extraction-functions) using the syntax `->'field'`. The two meanings of the single arrow operator are treated the same by the binder, thus they share the same (low) precedence, necessitating extra parentheses in JSON expression with equality checks:
...
To make the transition smoother, the deprecation will happen in several steps over the next year. First, DuckDB 1.3.0 [introduces a new setting to configure the lambda syntax](https://github.com/duckdb/duckdb/pull/17235):
...
To serialize a list of strings with the old behavior, use the [`array_to_string` function](https://duckdb.org/docs/stable/sql/functions/list.html#array_to_string):
...
DuckDB already supported [`TRY_CAST`](https://duckdb.org/docs/stable/sql/expressions/cast.html#TRY_CAST), which was trying to cast a value but did not fail the query if this was not possible. For example:
...
The [`ATTACH OR REPLACE` clause](https://github.com/duckdb/duckdb/pull/15355) allows you to replace a database, so you can swap a database on the fly. For example:
...
> #### Warning
> 
> **Update (2025-05-23).**The [UUID v7 implementation in DuckDB v1.3.0 is not consistent with the UUID standard](https://github.com/duckdb/duckdb/issues/17611), causing the timestamp values to be off. This means that the timestamps in the generated UUID v7 values will only be correct if they are used exclusively within DuckDB. Importing or exporting UUID v7 values from/to systems will yield incorrect timestamps. We [patched this bug](https://github.com/duckdb/duckdb/pull/17612) and the fix will soon be available in the preview builds and the [upcoming 1.3.1 patch release](https://duckdb.org/release_calendar.html#upcoming-releases).
...
DuckDB v1.3.0 brings a further boost to the popular [`COLUMNS(*)` expression](https://duckdb.org/docs/stable/sql/expressions/star.html#columns-expression). Previously, *unpacking* the entities into a list was possible by adding a leading `*` character:
...
We added a new specialized join operator as part of the [`spatial` extension](https://duckdb.org/docs/stable/core_extensions/spatial/overview.html), which greatly improves the efficiency of *spatial joins*, that is, queries that `JOIN` two geometry columns using specific spatial predicate functions, such as `ST_Intersects` and `ST_Contains`.
